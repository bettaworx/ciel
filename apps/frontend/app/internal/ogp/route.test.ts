...
const mockSafeFetch = vi.fn<(...args: unknown[]) => Promise<SafeFetchResult | SafeFetchError>>();
...
describe("GET /internal/ogp", () => {
  ...
  it("returns OGP data for a valid page", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://example.com/page");
    const res = await GET(req);
  ...
  it("returns 404 when no OGP metadata is found", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://example.com/empty");
    const res = await GET(req);
  ...
  it("returns error when safeFetch fails (SSRF blocked)", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://evil.com");
    const res = await GET(req);
  ...
  it("returns error when safeFetch fails with status", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://forbidden.com");
    const res = await GET(req);
  ...
  it("returns proper cache-control header on success", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://example.com");
    const res = await GET(req);
    expect(res.status).toBe(200);
  ...
  it("returns Twitter OGP data via syndication API for x.com tweet URL", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://x.com/jack/status/20");
    const res = await GET(req);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.data).toBeDefined();
  ...
  it("falls back to standard OGP when Twitter syndication fails", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://x.com/user/status/999");
    const res = await GET(req);
  ...
  it("falls back to standard OGP when Twitter syndication throws", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp?url=https://x.com/user/status/888");
    const res = await GET(req);
    expect(res.status).toBe(200);
    expect(mockSafeFetch).toHaveBeenCalled();
  });
...
describe("GET /internal/ogp/image", () => {
  ...
  it("returns 400 for non-http URL", async () => {
    const req = new Request("http://localhost/internal/ogp/image?url=data:image/png;base64,abc");
    const res = await GET(req);
    expect(res.status).toBe(400);
  });
  ...
  it("proxies an image successfully", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp/image?url=https://example.com/img.png");
    const res = await GET(req);
    expect(res.status).toBe(200);
  ...
  it("returns 413 when image exceeds size limit (5 MiB)", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp/image?url=https://example.com/huge.png");
    const res = await GET(req);
  ...
  it("defaults content-type to image/png when upstream omits it", async () => {
    ...
    const req = new Request("http://localhost/internal/ogp/image?url=https://example.com/noct.png");
    const res = await GET(req);
    expect(res.status).toBe(200);
