...
export async function GET(request: Request): Promise<NextResponse<OgpApiResponse>> {
	try {
		...
		if (!result.ok) {
			console.error('[OGP] safeFetch failed:', result.reason, 'url:', url);
		...
		try {
			...
			console.error('[OGP] Failed to read response body:', err);
		...
		console.error('[OGP] Unhandled error in OGP route:', err);
