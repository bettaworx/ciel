...
export function WebSocketDisconnectAlert({
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="fixed left-auto top-6 right-6 translate-x-0 translate-y-0 max-w-md shadow-none">
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-3">
            <WifiOff className="h-5 w-5 text-muted-foreground" />
            <span>{t("title")}</span>
          </AlertDialogTitle>
          <AlertDialogDescription className="text-left">{t("description")}</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
