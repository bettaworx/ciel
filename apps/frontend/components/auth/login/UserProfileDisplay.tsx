 ...
 */
export function UserProfileDisplay({ username, layout = "inline" }: UserProfileDisplayProps) {
  ...
  if (isLoading) {
    return (
      ...
      >
        ...
        />
        <div className={cn("flex flex-col gap-2", stacked ? "items-center" : "flex-1")}>
  ...
  return (
    <div
      className={cn("rounded-lg flex gap-3", stacked ? "flex-col items-center" : "items-center")}
    >
      {/* Avatar */}
      ...
      {/* User info */}
      <div className={cn("flex flex-col min-w-0", stacked && "items-center text-center")}>
        {user.displayName ? (
          <>
