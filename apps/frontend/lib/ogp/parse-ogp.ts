...
export function parseOgp(html: string, pageUrl: string): OgpData | null {
  ...
  const title = (ogTitle ?? twitterTitle ?? htmlTitle) || undefined;

  const description =
    ogMeta("og:description") ?? ogMeta("twitter:description") ?? ogMeta("description");
  ...
  const siteName = ogMeta("og:site_name") ?? ogMeta("twitter:site");

  const canonicalUrl =
    (ogMeta("og:url") ?? $('link[rel="canonical"]').attr("href")?.trim()) || undefined;
  ...
  return {
    ...
    imageHeight:
      absoluteImage && imageHeight && !Number.isNaN(imageHeight) ? imageHeight : undefined,
