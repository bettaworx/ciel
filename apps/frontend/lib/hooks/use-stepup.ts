...
export function useStepup({ onToken }: UseStepupOptions = {}) {
  ...
  const apply = useCallback(
    (result: Awaited<ReturnType<typeof stepup>>, failureKey: string): string | null => {
      if (result.ok === "mfa") {
        setChallenge(result);
        return null;
      }
  ...
  const run = useCallback(async (action: () => Promise<string | null>): Promise<string | null> => {
    setLoading(true);
    try {
      return await action();
    } catch {
  ...
  const submitPassword = useCallback(
    (password: string) =>
      run(async () => apply(await stepup(username, password), "settings.reauth.failed")),
