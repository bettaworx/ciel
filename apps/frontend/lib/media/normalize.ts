 ...
 */
export type ImageQualityMode = "none" | "dot-by-dot" | "performance" | "balance" | "quality";
...
export function pickVideoBitrate(input: {
  ...
  const source = durationSec > 0 ? (fileSize * 8) / durationSec - AUDIO_BITRATE : Infinity;
 ...
 */
export async function normalizeForUpload(file: File, opts: NormalizeOptions = {}): Promise<File> {
...
async function normalizeVideo(
  ...
  const frameRate = averagePacketRate > limits.maxFrameRate ? limits.maxFrameRate : undefined;
