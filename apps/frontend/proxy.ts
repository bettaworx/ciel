...
function backendHttpSources(): string[] {
  ...
  return [...originSource(apiBaseUrl), ...originSource(process.env.PUBLIC_BASE_URL)];
}
...
function buildCsp(nonce: string): string {
  const d = (key: string, fallback: string) => sanitize(process.env[key] || fallback);
  ...
  matcher: [
    {
      ...
      source: "/((?!_next/static|_next/image|favicon\\.ico|pwa/|icon|sw\\.js).*)",
      missing: [
